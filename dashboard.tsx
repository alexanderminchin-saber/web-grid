...
export default function App() {
  ...
  const [compactMode, setCompactMode] = useState(false); // If true, pane headers float/auto-hide
  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      // Toggle header with Ctrl + \ (Backslash)
  ...
  const findOpenSpot = (width, height) => {
    ...
    // Brute force search for non-overlapping spot
    while (!found && attempts < 2000) {
      ...
      if (!hasCollision) {
        ...
        checkX += step;
        // If we hit right edge, carriage return
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 overflow-hidden flex flex-col font-sans">
          ...
          <form onSubmit={addPane} className="flex-1 max-w-xl flex gap-2">
            <div className="relative flex-1">
              ...
              <input 
                ...
                className="w-full bg-slate-950 border border-slate-700 rounded-md py-2 pl-10 pr-4 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all"
                value={urlInput}
      ...
      <main ref={mainRef} className="relative flex-1 overflow-auto">
        ...
        {panes.map((pane) => (
          ...
          >
            {/* Pane Header:
              If compactMode is ON: it becomes absolute, transparent, and only shows on hover.
              If compactMode is OFF: it is relative (block) and always visible.
            */}
            <div 
              className={`
